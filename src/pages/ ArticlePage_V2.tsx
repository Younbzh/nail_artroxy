...
function ArticlePage() {
  ...
  useEffect(() => {
    async function loadArticle() {
      ...
      setLoading(true);
      try {
        // Importer dynamiquement le fichier .md
        const mdModule = await import(`../content/articles/${slug}.md?raw`);
        const rawContent = mdModule.default;
        ...
        if (match) {
          ...
          // Parser le frontmatter YAML manuellement
          ...
          setArticleContent({
            frontmatter: {
              title: frontmatter.title || article?.title || 'Article',
              slug: frontmatter.slug || slug,
              date: frontmatter.date || article?.date || '',
              category: frontmatter.category || article?.category || 'Actualités',
              readTime: parseInt(frontmatter.readTime) || article?.readTime || 5,
          ...
          // Pas de frontmatter, utiliser tout le contenu
          setArticleContent({
            frontmatter: {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-rose-50 via-pink-50 to-purple-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/blog" className="text-pink-600 hover:text-pink-700 font-semibold">
            ← Retour au blog
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-rose-50 via-pink-50 to-purple-50">
            ...
            <>
              ...
              <div className="mb-8">
                ...
                <div className="flex items-center gap-6 text-gray-500">
                  ...
                  <span className="flex items-center gap-2">
                    ...
                    {articleContent?.frontmatter.readTime || article.readTime} min de lecture
                  </span>
                </div>
              </div>
              ...
              {(articleContent?.frontmatter.image || article.image) && (
                <div className="rounded-2xl overflow-hidden shadow-2xl mb-8">
                  <img 
                    src={articleContent?.frontmatter.image || article.image} 
                    alt={articleContent?.frontmatter.title || article.title} 
                    className="w-full h-auto object-cover"
                    onError={(e) => {
                      const target = e.target as HTMLImageElement;
                      target.src = '/nail-art-sample.jpg';
                    }}
                  />
                </div>
              )}
                  ...
                  {articleContent?.content ? (
                      ...
                      dangerouslySetInnerHTML={{ 
                        __html: articleContent.content
                          ...
                          .map(line => {
                            // Convertir ## en h2
                            if (line.startsWith('## ')) {
                              return `<h2 class="text-2xl font-bold text-pink-900 mt-8 mb-4">${line.slice(3)}</h2>`;
                            }
                            // Convertir ### en h3
                            if (line.startsWith('### ')) {
                              return `<h3 class="text-xl font-bold text-pink-800 mt-6 mb-3">${line.slice(4)}</h3>`;
                            }
                            // Paragraphes
                    ...
                    <div className="border-t border-pink-100 pt-6">
                      <p className="text-gray-600 italic">
                        📝 Le contenu complet de cet article est en cours de rédaction. Revenez bientôt pour découvrir tous mes conseils détaillés !
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div className="mb-8">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {(articleContent?.frontmatter.tags || article.tags).map((tag, i) => (
                    <span key={i} className="px-3 py-1.5 bg-pink-100 text-pink-700 text-sm font-medium rounded-full">
                      {tag}
                    </span>
                  ))}
                </div>
              ...
              <div className="bg-gradient-to-br from-pink-100 via-rose-100 to-purple-100 rounded-3xl p-8 text-center shadow-lg">
                <h3 className="text-2xl font-bold text-pink-900 mb-4 font-serif" style={{ fontFamily: "'Bodoni Moda', serif" }}>
                  Envie d'ongles sublimes ?
                </h3>
                <p className="text-gray-700 mb-6">
                  Réservez votre prestation en ligne et bénéficiez de -15% pour votre première visite
                </p>
                <a 
      ...
      <style>{`
        ...
        .markdown-content strong { font-weight: 600; color: #831843; }
